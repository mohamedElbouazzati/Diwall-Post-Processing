...
output_file_path = './test_file.csv'  # Replace with the desired path for the output CSV file
...
for chunk in pd.read_csv(input_file_path, chunksize=chunk_size):
    ...
    selected_chunk = chunk.head(2 * lines_per_chunk) if (chunk['Packet_Nbre'] == 0).any() else chunk.head(lines_per_chunk)
